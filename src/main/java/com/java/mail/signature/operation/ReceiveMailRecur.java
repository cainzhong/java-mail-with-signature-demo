...
public class ReceiveMailRecur {
	...
	public static void main(String args[]) {
		...
		Map<String, String> map = convertJsonToMap(jsonObject);
		ReceiveMailRecur receiveMail = new ReceiveMailRecur(map.get("host"), map.get("port"), map.get("auth"), map.get("protocol"), map.get("username"), map.get("password"), map.get("proxySet"), map.get("proxyHost"), map.get("proxyPort"));
		try {
	 ...
	 * @param proxyPort
	 */
	public ReceiveMailRecur(String host, String port, String auth, String protocol, String username, String password, String proxySet, String proxyHost, String proxyPort) {
		this.host = host;
		this.port = port;
		this.auth = auth;
	...
	public Part recurMulti(Part part) throws IOException, MessagingException {
		if (part.getContent() instanceof Multipart) {
			Multipart multi = (Multipart) part.getContent();
			for (int i = 0; i < multi.getCount(); i++) {
				Part tempPart = this.recurMulti(multi.getBodyPart(i));
				this.partList.add(tempPart);
			}
		}
